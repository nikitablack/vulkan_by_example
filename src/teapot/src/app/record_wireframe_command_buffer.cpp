...
MaybeAppData record_wireframe_command_buffer(AppData data)
{
	...
	assert(data.wireframePipeline);
	assert(data.positionsBuffer);
	assert(data.wireframeCommandBuffers.size() == data.swapChainFramebuffers.size());
	
	for (size_t i = 0; i < data.wireframeCommandBuffers.size(); i++)
	{
		VkCommandBuffer const commandBuffer{data.wireframeCommandBuffers[i]};
		...
		vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, data.wireframePipeline);
